...
if (isset($_POST['registerbtn'])) {
    ...
    $email_query = "SELECT * FROM basic_user WHERE email='$email' ";
    ...
    if (mysqli_num_rows($email_query_run) > 0) {
        ...
        if ($password === $conpassword) {
            $query = "INSERT INTO basic_user (username,email,password) VALUES ('$email','$email','$password')";
...
if (isset($_POST['login_btn'])) {
    ...
    $query = "SELECT * FROM basic_user WHERE email='$email_login' AND password='$password_login' LIMIT 1";
    $query_run = mysqli_query($connection, $query);
...
if (isset($_POST['update_dark'])) {
    ...
    $query = "UPDATE basic_user SET dark='$dark' WHERE id='$id' ";
    $query_run = mysqli_query($connection, $query);
    if ($query_run) {
...
if (isset($_POST['update_team'])) {
    ...
    $query = "UPDATE basic_user SET team='$team' WHERE id='$id' ";
    $query_run = mysqli_query($connection, $query);
    if ($query_run) {
        $_SESSION['status'] = "Colour mode has been changed";
        $_SESSION['status_code'] = "success";
